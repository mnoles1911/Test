...
# Y coordinate (in voxel units, 10 vox/m) of the bedrock layer. Edits
...
const WORLD_FLOOR_WORLD_Y: float = float(WORLD_FLOOR_VOXEL_Y) / 10.0
# Same value in world-space metres (10 vox/m). Used by world-coord
# AABB checks below.
#
...
# explosive sphere at 10 vox/m costing ~33500 voxels, this lets
# ~6 sphere edits drain in a single physics frame, so rapid
...
func _apply_edit(cmd: Dictionary) -> void:
	...
	# the inverse transform (which for our scale-only setup is *=10).
	#
	...
	match cmd["type"]:
		...
		"set":
			...
			# sphere clears ~5 cubes at 10 vox/m — feels like a
...
# Voxel scale: 10 voxels per meter — locked in 2026-05-03 as the
...
# Chunk side length in meters — derived. At 10 vox/m, each chunk is
# 16/10 = 1.6 m on a side.
...
func _estimate_voxel_cost(cmd: Dictionary) -> int:
	...
	#
	# The multiplier is voxels-per-cubic-meter at our scale:
	# 10 vox/m on each axis = 10^3 = 1000 voxels per m^3.
	const VOXELS_PER_CUBIC_METER: float = 1000.0
	match cmd["type"]:
		"sphere":
