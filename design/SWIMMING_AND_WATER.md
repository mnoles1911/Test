...
- **Underwater visibility (v2 — volumetric fog + sun god rays, implemented 2026-05-19):** `scripts/UnderwaterFilter.gd` is still a `CanvasLayer` child of Player3D, but it now drives Godot 4's `WorldEnvironment.volumetric_fog_*` and the Sun `DirectionalLight3D.light_volumetric_fog_energy` to produce true 3D in-scene visibility murk + sunbeams penetrating the water column. The `TintRect` `ColorRect` is kept at alpha `0.10` as a subtle blue colour grade on top of the fog (so close-up water still reads tonally as "underwater" before the distance term kicks in).
- **What changes on submerge:** `Player3D._update_water_state()` calls `set_active(true)` (unchanged API). UnderwaterFilter starts a `Tween` over `transition_seconds` (default 0.5 s) that ramps `env.volumetric_fog_density` up, recolours `volumetric_fog_albedo` and `volumetric_fog_emission` to the underwater preset, and pushes `sun.light_volumetric_fog_energy` from 0 toward the god-ray dial (≈ 6.0 at noon). On `set_active(false)`, the tween reverses back to the at-rest above-water atmospheric haze (the baseline `volumetric_fog_density = 0.015` etc. already in `scenes/World3D.tscn`).
- **Day/night coupling (new):** while submerged, UnderwaterFilter's `_process` lerps the underwater preset between `*_noon` and `*_night` anchor values every frame, using `clamp(sun.light_energy / sun_noon_energy_ref, 0, 1)` as the mix factor. `DayNightCycle.gd` is already animating `sun.light_energy` 0 → 2.2 → 0 across the day, so visibility opens at sunrise and closes at sunset *without* any new signal or coupling to `DayNightCycle`. At midnight: fog density ≈ 0.70, god rays ≈ 0.2, near-inky cobalt-black. At noon: fog density ≈ 0.30, god rays ≈ 6.0, deeper cobalt blue with visible sunbeams. (Initial defaults were 0.40 / 0.18 with teal albedo; tightened 2026-05-19 because designer reported ~12–30 m noon visibility — too clear. Current values target ~5–6 m noon / ~2 m midnight.)
- **Why this co-exists with `DayNightCycle` safely:** `DayNightCycle.gd` only writes the *classic* `env.fog_light_color` + `env.fog_density` (10 Hz). It never touches `volumetric_fog_*` or `sun.light_volumetric_fog_energy` — UnderwaterFilter owns those properties exclusively (verified by full-file grep). The earlier v1 "Future polish" note about replacing the flat ColorRect with a screen-space shader has been **superseded by this volumetric-fog approach** — a screen shader cannot produce real god rays and cannot react to the sun's actual position.
- **Setup contract (scenes/World3D.tscn):** the `WorldEnvironment` node is in the `world_environment` group; the `Sun` `DirectionalLight3D` is in the `sun_light` group; the Environment has `volumetric_fog_enabled = true`. UnderwaterFilter looks both up via `get_tree().get_first_node_in_group(...)` in `_ready()`. If either is missing (e.g. dev scene without day/night), it degrades gracefully — the tint rect still works.
- **Tunable knobs (all `@export` on `UnderwaterFilter.gd`):** `underwater_fog_density_noon/_night`, `underwater_fog_albedo_noon/_night`, `underwater_fog_emission_noon/_night`, `underwater_god_ray_energy_noon/_night`, `surface_fog_density`, `surface_fog_albedo`, `transition_seconds`, `sun_noon_energy_ref`.
- **Underwater visibility (v3 — back-face surface + variety + particulates + wobble, implemented 2026-05-19, PR #232):** Designer feedback ("fog feels backwards: clear seabed, pale ceiling") pointed at `water.gdshader` rendering its underside with the front-face code path (Fresnel sky reflection + sky-distance depth-fade) — a pale washed ceiling with no visible flow. v3 layered four interlocking pieces:
  - **Water back-face branch (`assets/shaders/water.gdshader`):** `if (!FRONT_FACING && debug_mode == 0)` renders the underside as `deep_water_color` + a `pow(dot(view_world, -sun_direction_world), underside_sun_glint_power) * underside_sun_glint_strength` bright spot toward the sun. Glint shimmers mildly with the wave-perturbed normal so the surface visibly moves from below. UnderwaterFilter pushes the live `sun_direction_world` into the shader each frame so the glint tracks day/night. Early-return preserves all front-face logic; non-zero debug modes fall through.
  - **Match-the-deep-water-color:** `underwater_fog_albedo_noon = Color(0.02, 0.06, 0.11)` — *exactly* the surface shader's `deep_water_color` so the underwater fog and the body-of-water deep tint are the SAME colour. Consistency across all three "I'm seeing water" rendering paths.
  - **Animated FogVolume (`assets/shaders/underwater_fog.gdshader`):** `shader_type fog` with 3 octaves of scrolling 3D value noise modulating `DENSITY` (asset-free hash-based noise; three different scales 1.0/1.9/3.7 + drift velocities so the octaves never re-phase). Hosted by an `UnderwaterFogVolume` node in `World3D.tscn` (shape=WORLD, group `underwater_fog_volume`). Toggled via `.visible` on submerge. Adds drifting denser/lighter patches on top of the env's flat baseline — water feels alive instead of being uniform murk.
  - **Drifting particulates:** `GPUParticles3D` "UnderwaterParticulates" sibling of UnderwaterFilter under Player3D (group `underwater_particulates`). ~200 small unshaded billboards, slow upward drift (~0.05 m/s, gravity +0.012), 8 s lifetime, mild turbulence. `local_coords = false` means the emission *origin* follows the player but the particles themselves live in world space — exactly the "I'm moving through them" feel, not fireflies. Toggled via `.emitting` (not `.visible`) so motes finish their lifetime gracefully when surfacing.
  - **Screen-wobble + chromatic aberration overlay (`assets/shaders/underwater_overlay.gdshader`):** `shader_type canvas_item` on the TintRect. Slow noise-driven SCREEN_UV displacement (~6 px at 1080p, 0.6 Hz) + RGB sampled at tiny offsets in the wobble direction (~1.5 px CA). Mixes the distorted scene with the underwater tint. v1's flat `ColorRect.color` remains as a fallback if the shader fails on a player's GPU.
  - **`volumetric_fog_length: 80 → 48`** in `scenes/World3D.tscn` Environment — hard view-distance cap so murk closes faster in all directions.
- **Future polish (still deferred):** see "Deferred underwater polish" section in `design/WATER_SHADER_V3_PLAN.md` Phase 4b/c/d (caustics, per-biome fog, planar reflection, audio coupling).
