...
**Voxel scale:** **10 voxels per meter (each block = 10 cm)** — migrated 2026-06-14 from the old 6 vox/m (16.7 cm, locked 2026-05-03) to match the asset standard used by the voxel studios (`tools/voxel_*`). Chunky enough to read as cubic, finer than Minecraft's 1 m blocks. All terrain/prop assets (studio exports, MagicaVoxel props, building exports) are authored at this scale. `VoxelLodTerrain.transform.scale = 0.1`. The world keeps the SAME physical size in metres — generator voxel-coordinates were scaled ×10/6 and noise frequencies ×6/10, so only the resolution got finer (physical player movement, capsule, gravity all unchanged). **Characters/NPCs** are authored at a finer **20 voxels per meter** (they ship as rigged `.glb` meshes via Blender→Mixamo, NOT terrain voxels). See `DESIGNER_TODO.md` §8 for the in-editor follow-ups (recompile the C++ extension, wipe the `user://` voxel sqlite caches, playtest).
...
**Scale:** 1 MagicaVoxel block = 0.10 m in Godot (10 voxels per meter). A 1.8 m doorway = 18 blocks tall.
