...
- **2026-05-19 (underwater fog + god rays, branch `water-shader-v3-p4-underwater`):** `scripts/UnderwaterFilter.gd` now drives `WorldEnvironment.volumetric_fog_*` and `Sun.light_volumetric_fog_energy` on submerge, tweening between an above-water atmospheric haze and an underwater preset *lerped between `*_noon` and `*_night` anchors* every frame via `clamp(sun.light_energy / sun_noon_energy_ref, 0, 1)`. Day/night-coupled visibility (12 m at noon, 3–4 m at midnight) and sun-driven god rays through the water column. Co-exists safely with `DayNightCycle` (DNC only writes classic `fog_*`, never `volumetric_fog_*`). Scene contract: WorldEnvironment in group `world_environment`, Sun in group `sun_light`, `volumetric_fog_enabled = true` in `scenes/World3D.tscn`. Fills `WATER_SHADER_V3_PLAN.md` Phase 4a (caustics + per-biome remain Phase 4b).
