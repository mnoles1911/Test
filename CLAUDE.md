...
- **Milestone 5-3D (largely complete, 2026-05-03):** Core destructible-voxel slice is shipping and verified in-game.
  ...
  - Water surface upgrade (2026-05-04): custom sine-sum vertex-displacement shader (`assets/shaders/water.gdshader`), wind plumbing on `WaterVolume.gd` for future `WeatherManager` hookup, vertical swim controls (Space ascends, Crouch dives), 2 s breath recovery delay on surfacing, blue-green underwater tint via `UnderwaterFilter` `CanvasLayer` ✅
...
- `WaterVolume.gd` — script attached to any Area3D in the `water_volume` group. Exposes `surface_y` (world-space), `get_current_velocity()` for river flow, and wind state (`wind_direction`, `wind_strength`) that drives the surface shader's wave direction and amplitude. `WaterVolume.set_wind(dir, strength)` is the public hook the future `WeatherManager` will call to update both at once. Player3D polls overlapping volumes per-frame for swim physics.
- `assets/shaders/water.gdshader` + `assets/shaders/water_material.tres` — custom sine-sum vertex-displacement shader for animated water surfaces. Two summed sine waves displace `VERTEX.y`; wind biases the dominant wave direction and scales overall amplitude. World-space XZ as wave domain so adjacent water bodies stay phase-aligned. One shared `ShaderMaterial.tres` referenced by every water scene.
