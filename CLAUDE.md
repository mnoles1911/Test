...
- **2026-05-19 (underwater fog + god rays + variety + back-face fix, PR #232 on branch `water-shader-v3-p4-underwater`):** Built in 5 commits. *Initial:* `scripts/UnderwaterFilter.gd` drives `WorldEnvironment.volumetric_fog_*` and `Sun.light_volumetric_fog_energy` on submerge, tweening between an above-water atmospheric haze and an underwater preset *lerped between `*_noon` and `*_night` anchors* every frame via `clamp(sun.light_energy / sun_noon_energy_ref, 0, 1)` — day/night-coupled visibility + sun-driven god rays through the water column. Co-exists safely with `DayNightCycle` (DNC only writes classic `fog_*`, never `volumetric_fog_*`). *C1:* `water.gdshader` gets a `FRONT_FACING` back-face branch — looking up from underwater renders `deep_water_color` + a sun-direction-driven glint (UnderwaterFilter pushes live `sun_direction_world`), so the underside reads dark with a visible bright sun spot and animated surface motion instead of the v9 pale-sky ceiling. `underwater_fog_albedo_noon` set to exactly `deep_water_color (0.02,0.06,0.11)` for cross-system consistency; densities 0.55/1.10; `volumetric_fog_length: 80 → 48`. *C2:* `assets/shaders/underwater_fog.gdshader` (`shader_type fog`) + `UnderwaterFogVolume` node modulates DENSITY with 3-octave scrolling 3D noise — drifting variety on top of the flat baseline. *C3:* `GPUParticles3D UnderwaterParticulates` under Player3D, ~200 small unshaded billboards with slow upward drift + `local_coords=false` (player swims through, motes stay in world). *C4:* `assets/shaders/underwater_overlay.gdshader` (`shader_type canvas_item`) on TintRect — slow noise-driven SCREEN_UV wobble + tiny chromatic aberration. Three new scene groups: `world_environment`, `sun_light`, `underwater_fog_volume`, `underwater_particulates`. Phase 4b (caustics), 4c (per-biome), 4d (planar reflection), 4e (audio) deferred in `design/WATER_SHADER_V3_PLAN.md`.
