...
**Done:** `CubicHeightmapGeneratorCpp`, `CopperIslesHeightmapGeneratorCpp`, `HeightmapGeneratorBase` (PR #203 — extracted ~500 shared lines), `WaterChunkMesherCpp` (PR #214 — greedy 2D run-merge + ArrayMesh build), `VoxelGravityCpp` + `EmissiveLightCpp` (branch `perf/vgm-elm-cpp-ports`, 2026-05-26 — autoloads delegate hot path to C++; `gravity` + `emissive` headless selectors set-for-set parity-green; in-engine smoke pending; PR pending).

**Next targets, payoff-vs-effort order (post `perf/vgm-elm-cpp-ports`):**

1. **`WaterFlowManager.gd`** flow tick (M). 4 Hz scan over chunks within 20 m, per-voxel byte read/write. `WaterByteCodec` is POD → mostly buffer iteration. Lower priority now that streaming isn't competing for main-thread budget. Re-measure with a fresh capture before porting — VGM/ELM were the named hotspots in the 2026-05-26 capture; their absence may surface a new top-of-list candidate.
