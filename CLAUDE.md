...
Outstanding pickups: Blender Roland model, MagicaVoxel prop exports, surface decoration pass, ambient weather audio, region-boundary profile auto-swap, LOD1+ water surface line artefact (shader fix), `VoxelGravityManager` C++ port, `EmissiveLightManager` sweep optimisation. See `DESIGNER_TODO.md`.
...
**Next targets, payoff-vs-effort order (post 2026-05-26 capture):**

1. **`VoxelGravityManager.gd`** flood-fill (S-M). **NOW THE #1 PERF HOTSPOT** — 16 m local BFS for unsupported voxels produces 130 ms blast spikes on routine powder-charge use (three 100+ ms freezes per play session in the post-streaming-fix capture `profile_capture_257416.json`). Clean port — pure byte iteration, no SceneTree access. The cubic-generator port pattern (`extensions/voxel_gen/`) applies directly.
2. **`EmissiveLightManager.gd`** periodic vicinity sweep (S). Max 18 ms / 31 spike frames in the same capture; fires ~1 Hz. Three options: (a) chunk the sweep across N frames (~15 min, easiest), (b) move bulk `VoxelTool.copy()` + clustering to a worker thread (~45 min), (c) C++ port (~1-2 hr).
3. **`WaterFlowManager.gd`** flow tick (M). 4 Hz scan over chunks within 20 m, per-voxel byte read/write. `WaterByteCodec` is POD → mostly buffer iteration. Lower priority now that streaming isn't competing for main-thread budget.
