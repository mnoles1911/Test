...
Outstanding pickups: Blender Roland model, MagicaVoxel prop exports, surface decoration pass, ambient weather audio, region-boundary profile auto-swap, LOD1+ water surface line artefact (shader fix), regen 5 PHOTO-flagged voxel textures with a pixel-art-specific generator (`grass_top`, `gravel_all`, `log_side`, `leaves_all`, `copper_ore_all`), Zylann `ShaderMaterialPool::recycle` assertion in the F11 LOD-debug toggle path (introduced in PR #240; surfaced more visibly post-#241 when the F11 toggle was exercised more). See `DESIGNER_TODO.md`.
