...
- **2026-05-22 (DistantTerrain — streaming smooth-heightmesh LOD overhaul, branch `feat/distant-terrain-lod`):** Fixed LOD terracing / hard LOD seams — the #1 deferred visual problem. Root cause: `VoxelMesherBlocky` has no transition cells, so every Zylann LOD shell cracked + stepped, and blocky LOD downsampling is inherently terraced (an LOD3 cube is ~1.3 m). Fix: a **two-layer terrain**. *Near (~0–450 m):* Zylann blocky voxels — still editable — with `lod_count` 6→4 and `VoxelViewer.view_distance` 8000→2700 voxels so the worst far LODs never stream (the real terracing fix; Copper Isles keeps `lod_count`=9 — view_distance alone makes its high LODs dormant, dodging the bake-cache contract). *Far:* `DistantTerrain` — a streaming smooth heightmesh. New C++ `DistantTerrainMesher` (`extensions/voxel_gen/`, a 1:1 port of the retired `SkirtBaker.gd` — elevation/slope/jitter palette, spliced cliff walls, central-difference normals — plus a perimeter skirt apron) builds chunks from the generator's `compute_ground_y`; returns raw arrays so it can run off-thread. New `scripts/DistantTerrainManager.gd` (Node3D, **not** an autoload — created by `World3DBootstrap`/`CopperIslesTestBootstrap` under the unscaled world root; loaded by path, **no `class_name`**, so the bootstraps stay headless-parseable) streams concentric LOD rings (4 m quads near → 128 m far, 6 rings) that follow the player: set-difference recentering, budgeted main-thread builds, a `MeshInstance3D` pool, an inner-radius cull (no smooth mesh inside the blocky band → digging never exposes it), `MAX_LIVE_CHUNKS` cap, and a Bayer-dither LOD-swap cross-fade via `assets/shaders/distant_terrain.gdshader`. Seam-free by construction: aprons plug T-junction cracks with no neighbour-LOD knowledge, the dither hides LOD pops, both layers share one height function (the smooth mesh sits ~1.5 m below true ground so blocky wins the depth test in the overlap). Retired the baked static `HorizonSkirt` + `SkirtBaker` + the 21 MB `copper_isles_skirt.res` + the "Bake horizon skirt" tool button. Headless-gated each sub-phase via a new `distant` selector (`tools/headless/`): C++ output bit-identical to the committed `SkirtBaker`-derived baseline (`tools/headless/distant_parity_baseline.json`), apron purely additive, shader compiles. Visual verification (no terracing, seamless recentering, the blocky↔smooth handoff) + the final view_distance/lod_count/inner_cull/fog tuning are the designer's in-editor pass (F12 LOD draws) — see `DESIGNER_TODO.md`. **Fog note:** `env.fog_density` is fully runtime-driven (`WeatherManager` per-state → `DayNightCycle.set_fog_override`), so there is no static fog retune — atmosphere tuning rides the in-editor weather pass.

Outstanding pickups: Blender Roland model, MagicaVoxel prop exports, surface decoration pass, ambient weather audio, region-boundary profile auto-swap, DistantTerrain in-editor tuning pass. See `DESIGNER_TODO.md`.
