...
| `voxel_tree_studio/index.html` (Three.js, browser) | Procedural voxel-tree **shape** iterator. Sliders → live preview → Export JSON (`{x,y,z,m}`, m=10 log/11 leaves). Shape-only; final look verified in Godot. Importer is a separate, not-yet-built step. |
