...
| `voxel_character_studio/` (browser) | T-pose **biped** generator. `character_builder.js` (pure): parametric joint skeleton + axis-aligned tapered-box body parts (head/neck/torso/pelvis/arms/hands/legs/feet), build x≥0 + mirror for symmetry. 18 vox/m. Material ids 40–49 (preview/vertex-bake only — chars are MESHES, not terrain, so NO engine registration). Export `mira-thal-voxel-character` w/ **embedded RGB palette**. Claude-vision fit (`claude_character.js`). Downstream: `import_voxel_json.py` (reads embedded palette) → Blender → **Mixamo** → `.glb`. Replaces Nano-Banana→Meshy step. |
