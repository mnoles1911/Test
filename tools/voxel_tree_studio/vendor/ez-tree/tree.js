...
export class Tree extends THREE.Group {
  ...
  generate() {
    ...
    // VENDOR ADDITION (Voxel Tree Studio): capture the branch skeleton +
    // leaf anchor points so the voxelizer can rasterize cubes from the
    // centerlines. `skeleton` = one entry per branch: a polyline of section
    // centerline points with per-section radius. `leafAnchors` = leaf origin
    // points. Both are plain numbers (no THREE objects) so they transfer to a
    // worker cheaply. This does not affect ez-tree's mesh output.
    this.skeleton = [];
    this.leafAnchors = [];
  ...
  generateBranch(branch) {
    ...
    // VENDOR ADDITION (Voxel Tree Studio): record this branch's centerline
    // polyline (section origins) + per-section radii for voxelization.
    this.skeleton.push({
      level: branch.level,
      points: sections.map((s) => [s.origin.x, s.origin.y, s.origin.z]),
      radii: sections.map((s) => s.radius),
    });
  ...
  */
  generateLeaf(origin, orientation) {
    // VENDOR ADDITION (Voxel Tree Studio): record the leaf anchor point so the
    // voxelizer can grow connected leaf clumps from it.
    this.leafAnchors.push([origin.x, origin.y, origin.z]);
